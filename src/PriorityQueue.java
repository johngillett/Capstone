...
public class PriorityQueue {
	...
	private Map<Student, Integer> location;
	private List<Pair<Integer, Student>> heap;
	...
	public PriorityQueue(boolean isMax) {
		location = new HashMap<Student, Integer>();
		heap = new ArrayList<Pair<Integer,Student>>();
		this.isMax = isMax;
	}
	 ...
	 *  
	 */
	public void push(int priority, Student element) {
		// element must be unique
		assert(!location.containsKey(element));		
		
		// add the new element to the end of the list
		heap.add(new Pair<Integer,Student>(priority, element));
	...
	public void printHeap() {
		for(int i = 0; i < heap.size(); ++i) {
			Pair<Integer, Student> pair = heap.get(i);
	...
	public void printMap() {
		for(Map.Entry<Student, Integer> entry : location.entrySet()){
	...
	private int pushDown(int start_index) {
		...
		while(hasTwoChildren(curr)) {			
			...
			if(isMax && heap.get(curr).priority >= heap.get(swap_index).priority) {
				break;
			}
		...
		if(isMax && l < heap.size() && heap.get(l).priority > heap.get(curr).priority){
			swap(curr, l);
			curr = l;
		}
		if(!isMax && l < heap.size() && heap.get(l).priority < heap.get(curr).priority){
	...
	private void swap(int i, int j) {
		Pair<Integer, Student> temp = heap.get(i);

		heap.set(i, heap.get(j));
		heap.set(j, temp);

		Student key = heap.get(j).element;
		location.put(key, j);
